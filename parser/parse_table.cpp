...
int main() 
{
    Parser parser("TYPE ID ( TYPE ID ) { IF ( ID LTE NUM ) RETURN NUM ; ELSE RETURN ID * ID ( ID - ) ; } TYPE ID ( e ) { TYPE ID ; ID EQ NUM ; WHILE ( ID LTE NUM ) { ID ( ID ) ; ID ( ID ( ID ) ) ; ID ( e ) ; ID EQ ID + NUM ; } }", "tableP2", "rules");
